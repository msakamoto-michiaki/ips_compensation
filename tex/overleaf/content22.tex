\section{波長依存性評価（Dispersion: mono \& W-ISO）}
...
本表は \texttt{dispersion\_summary\_CRavg4.csv} 相当であり，
以降の議論は本表に基づく．
...
\subsection{波長依存プロット（図）}
図\ref{fig:lambda_dep}は ${\rm CR}00(\lambda)$ と
$\overline{{\rm CR}}_{30}(\lambda)$ の波長依存性を示す
（\texttt{plot\_lambda\_vs\_CR00\_and\_CRavg4\_mono.png}）．
なお図は別途作成するため，本稿では挿入箇所のみを示しコメントアウトする．
...
\]
まで落ちる．
すなわち，\textbf{正面暗状態を改善する方向（長波長側）と，
...
特に表\ref{tab:disp_summary}の 450--510~nm では
${\rm CR}(30,+45)$ と ${\rm CR}(30,-45)$ の大小関係が大きく入れ替わり，
一方の方位が高 ${\rm CR}$ に振れることで平均値が大きく変動している．

\subsection{RGB/W の ISO 分布（図）}
代表波長 B/G/R（450/546/610~nm）の単色 ISO と，
白色（W）ISO を図\ref{fig:iso_rgbw}に示す（図は別途作成するためコメントアウト）．
...
図\ref{fig:iso_rgbw}（挿入予定）では，B（450~nm）と G（546~nm），R（610~nm）で
...
\end{itemize}
など，\textbf{波長依存性を消す（または抑える）ような補償フィルムスタック設計}が必要である．
