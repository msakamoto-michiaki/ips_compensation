...
\subsection{結果}
\paragraph{中心（$\Delta=0$）での ${\rm CR}$ と初期非対称}
中心条件（$\Delta=0$）では，
\[
...
\subsection{$\Delta\in[-3^\circ,+3^\circ]$ 全点の数値（表1）}
...
\paragraph{方位対称化点（$\Delta\simeq +0.5^\circ$）と ${\rm CR}00$ の挙動}
表\ref{tab:allpoints}より，$\Delta=+0.5^\circ$ では
\[
...
一方で ${\rm CR}00$ は
\[
{\rm CR}00(0)=19762.9 \ \rightarrow\ {\rm CR}00(0.5^\circ)=16874.5
\]
と低下する．
すなわち，連動回転には「方位非対称を弱める $\Delta$」が存在し得る一方，
それが必ずしも正面暗状態の最適条件と一致しないことが示される．

\paragraph{符号依存性（$\Delta>0$ と $\Delta<0$）}
負側（$\Delta<0$）では $A_{45}$ が $+3$ dB 超まで増大し得るのに対し，
正側（$\Delta>0$）では $A_{45}$ が負へ反転し，$-2.8$ dB 程度まで変化する．
これは「貼りずれの符号」により，$\pm\phi$ の優劣が入れ替わることを意味する．
連動回転は相対角を保存するため単独回転より挙動が滑らかになり得るが，
符号依存性は残り，工程の系統オフセット（片寄り）を含めて管理する必要がある．
...
\begin{enumerate}
\item best 近傍条件でも，$\theta=30^\circ$ の 4方位 ${\rm CR}$ は完全対称ではなく，
中心で $A_{45}(0)\simeq +1.04$ dB，$A_{135}(0)\simeq -1.04$ dB の初期非対称が存在する．
\item $\Delta=+0.5^\circ$ では $A_{45}\approx A_{135}\approx 0$ となり方位対称化される一方，
${\rm CR}00$ は中心より低下する．
\item $\Delta$ の符号により ${\rm CR}_{30,+\phi}$ と ${\rm CR}_{30,-\phi}$ の優劣が反転し，
$|A_{45}|,|A_{135}|$ は最大で約 $3.5$ dB 程度まで増大し得る．
従って公差設計では ${\rm CR}00$ と $\min {\rm CR}_{30,\phi}$ に加え，
