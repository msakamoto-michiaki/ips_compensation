...
def _phi_to_coltag(phi_deg: float) -> str:
    """Convert a phi value to a safe column tag, e.g. +45 -> p45, -135 -> m135, 22.5 -> p22p5."""
    # keep one decimal if needed; replace '.' with 'p' to stay filesystem/CSV friendly.
    sgn = "p" if phi_deg >= 0 else "m"
    v = abs(float(phi_deg))
    if abs(v - round(v)) < 1e-9:
        core = f"{int(round(v))}"
    else:
        # up to 1 decimal place (matches typical use here); keep more if necessary
        core = f"{v:.1f}".rstrip("0").rstrip(".")
        core = core.replace(".", "p")
    return f"{sgn}{core}"


def _asymmetry_dB(cr_plus: float, cr_minus: float) -> float:
    """10*log10(CR(+phi)/CR(-phi)). Returns NaN if invalid."""
    import numpy as _np
    if cr_plus is None or cr_minus is None:
        return _np.nan
    if cr_plus <= 0 or cr_minus <= 0:
        return _np.nan
    return float(10.0 * _np.log10(cr_plus / cr_minus))


def _spread_dB(cr_list) -> float:
    """10*log10(max/min) over a list of CR values. Returns NaN if invalid."""
    import numpy as _np
    vals = [float(x) for x in cr_list if x is not None and float(x) > 0]
    if len(vals) == 0:
        return _np.nan
    return float(10.0 * _np.log10(max(vals) / min(vals)))
...
def main():
    ...
    for idx, d in enumerate(deltas):
        ...
        # --- additional per-phi columns (stable tags) ---
        for ph in mon_phis:
            tag = _phi_to_coltag(float(ph))
            row[f"CR_W_t{int(args.mon_theta):d}_phi_{tag}"] = float(crang[ph])

        # --- asymmetry metrics at theta=mon_theta over phis ±45, ±135 (if present) ---
        def _pick_phi(target):
            for _k, _v in crang.items():
                if abs(float(_k) - float(target)) < 1e-9:
                    return float(_v)
            return None
        _cr_p45 = _pick_phi(+45.0)
        _cr_m45 = _pick_phi(-45.0)
        _cr_p135 = _pick_phi(+135.0)
        _cr_m135 = _pick_phi(-135.0)
        row[f"A_45_dB_t{int(args.mon_theta):d}"] = _asymmetry_dB(_cr_p45, _cr_m45)
        row[f"A_135_dB_t{int(args.mon_theta):d}"] = _asymmetry_dB(_cr_p135, _cr_m135)
        row[f"spread_dB_t{int(args.mon_theta):d}"] = _spread_dB([_cr_p45, _cr_m45, _cr_p135, _cr_m135])
    ...
    # plot CR(theta=mon_theta, phi) for selected phis
    fig = plt.figure(figsize=(5.8, 3.8), dpi=200)
    ax = fig.add_subplot(111)
    for ph in mon_phis:
        tag = _phi_to_coltag(float(ph))
        col = f"CR_W_t{int(args.mon_theta):d}_phi_{tag}"
        if col in dfo.columns:
            ax.semilogy(dfo["delta_deg"].values, dfo[col].values, marker="o", label=f"phi={float(ph):g} deg")
    ax.set_xlabel("delta (deg)")
    ax.set_ylabel(f"CR (W) at theta={args.mon_theta:g} deg")
    ax.grid(True, ls=":")
    ax.set_title(f"CR(theta,phi) vs delta ({args.tag}, mode={args.rot_mode})")
    ax.legend(fontsize=8, ncol=2)
    fig.tight_layout()
    fig.savefig(outdir / "plot_delta_vs_CR_t30_phis.png", bbox_inches="tight")
    plt.close(fig)

    # plot asymmetry metrics (dB) at theta=mon_theta
    A45_col = f"A_45_dB_t{int(args.mon_theta):d}"
    A135_col = f"A_135_dB_t{int(args.mon_theta):d}"
    S_col = f"spread_dB_t{int(args.mon_theta):d}"
    fig = plt.figure(figsize=(5.8, 3.8), dpi=200)
    ax = fig.add_subplot(111)
    if A45_col in dfo.columns:
        ax.plot(dfo["delta_deg"].values, dfo[A45_col].values, marker="o", label="A_45 (dB)")
    if A135_col in dfo.columns:
        ax.plot(dfo["delta_deg"].values, dfo[A135_col].values, marker="o", label="A_135 (dB)")
    if S_col in dfo.columns:
        ax.plot(dfo["delta_deg"].values, dfo[S_col].values, marker="o", label="spread S (dB)")
    ax.set_xlabel("delta (deg)")
    ax.set_ylabel(f"asymmetry / spread (dB) at theta={args.mon_theta:g} deg")
    ax.grid(True, ls=":")
    ax.set_title(f"Azimuthal asymmetry vs delta ({args.tag}, mode={args.rot_mode})")
    ax.legend(fontsize=8, ncol=2)
    fig.tight_layout()
    fig.savefig(outdir / "plot_delta_vs_asymmetry_dB.png", bbox_inches="tight")
    plt.close(fig)
