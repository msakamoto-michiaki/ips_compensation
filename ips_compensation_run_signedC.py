...
def build_stack_realistic(
    ...
    # U-C (optional)  --- signed thickness supported ---
    # Interpretation:
    #   - thickness magnitude = abs(dC_um)
    #   - sign(dC_um) flips the sign of birefringence (dn_C), i.e. positive/negative C-plate
    if dC_um is not None and abs(float(dC_um)) > 0:
        _dC = float(dC_um)
        _sgn = 1.0 if _dC >= 0.0 else -1.0
        dnC_eff = float(dn_C) * _sgn
        stack.append({"type":"C", "axis":[0,0,1], "d":abs(_dC)*1e-6,
                      "no":NO_BASE, "ne":ne_from_dn(NO_BASE, dnC_eff)})
    # TAC out
    for _ in range(int(tac_repeat)):
        stack.append(dict(tac_layer))
...
def plot_isocontrast_polar(thetas, phis, CR, title, out_path, legend_every=2, theta_ticks=None):
    """Backward-compatible wrapper.

    Historically this function directly plotted the ISO polar contour and used `theta_ticks`
    in the same unit as the radial coordinate. The rest of this codebase uses:
      - thetas, phis in degrees
      - polar radius in radians
      - theta_ticks specified in degrees

    To avoid unit mismatch (e.g. r-ticks showing 0.2, 0.4... rad), we route all calls
    to `_iso_contour_lines_polar`, which handles degree->radian conversion and labels.
    """
    return _iso_contour_lines_polar(
        thetas, phis, CR,
        title=title,
        out_path=out_path,
        thr=None,
                            ...
                            for pol_out in POL_PAIR_ROT_OUT_DEGS:
                                for rel_L in REL_ROT_LA_DEGS:
                                    for rel_U in REL_ROT_UA_DEGS:
                                        ...
                                        if DO_PLOTS:
                                            plot_isocontrast_polar(thetas, phis, CR, title=title, out_path=str(out_png, theta_ticks=theta_ticks))
...
def run_Ascale_sweep_check_cr100(out_dir="asweep_cr100_contour",
    ...
    for A_scale in A_scales:
        ...
        _iso_contour_lines_polar(thetas, phis, CR, title=title, out_path=str(out_png), thr=thr, cr_cap=cr_cap, theta_ticks=theta_ticks)
        results.append({
            "A_scale": float(A_scale),
    ...
    for dC_um in dC_list:
        ...
        if make_iso_plots:
            for case_name, dn_map in [("matched", DN_SCALE_MATCHED), ("mismatched", DN_SCALE_MISMATCHED)]:
                ...
                if use_contour:
                    _iso_contour_lines_polar(thetas, phis, CR, title=title, out_path=str(out_png), thr=None, cr_cap=iso_cr_cap, theta_ticks=None)
                else:
...
#        + ISO contour (no fill) for readability
# ============================================================
def _iso_contour_lines_polar(thetas_deg, phis_deg, CR, title="ISO-Contrast (Polar)", out_path=None, *,
                             thr=None, levels=(100, 250, 500, 750, 1000, 1500, 2000, 3000, 5000, 10000, 20000, 50000),
                             cr_cap=None, theta_ticks=None):
    """
    Polar ISO plot with contour *lines* (no fill).
    thetas_deg: 1D degrees (radius)
    ...
    # ensure threshold line is included in contour levels when provided
    if thr is not None:
        thr = float(thr)
        if thr > 0:
            levels = tuple(sorted(set([float(x) for x in levels] + [thr])))
    ...
    if theta_ticks is not None:
        _rt = [np.deg2rad(float(t)) for t in theta_ticks]
        ax.set_rticks(_rt)
        ax.set_yticklabels([str(int(t)) if float(t).is_integer() else str(t) for t in theta_ticks])
    ax.set_title(title, fontsize=9)
    fig.tight_layout()
    fig.savefig(out_path, dpi=220)
    plt.close(fig)
