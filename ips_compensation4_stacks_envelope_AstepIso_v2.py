...
def export_ReA_ReC_map(builder_fn, A_scales: np.ndarray, ReC_nms: np.ndarray,
                      A_bases, A_kinds,
                      theta_tgt: float, phi_tgt: float, c1, c2,
                      outdir: Path, tag: str,
                      use_log_cr: bool = True):
    """
    Export a 2D map over (ReA, ReC) at the target view angle.

    For each (A_scale, ReC) grid point, this computes:
      CR(theta_tgt, phi_tgt) with the best choice over (A_base, A_kind)

    Outputs:
      - outdir/ReA_ReC_map_<tag>.csv
          long-form table: ReA_nm, ReC_nm, best_CR, best_A_base_deg, best_A_kind
      - outdir/ReA_ReC_grid_<tag>.csv
          pivot table: rows=ReA_nm, cols=ReC_nm, values=best_CR
      - outdir/ReA_ReC_map_<tag>.png
          contour plot (log10(CR) by default)
    """
    import matplotlib.pyplot as plt

    # grid arrays
    ReA_nms = (float(ips.RE_A_EACH_BASE_NM) * A_scales).astype(float)
    ReC_nms = np.array(ReC_nms, dtype=float)

    # compute best_CR over A_base/A_kind at each grid point
    CR_best = np.full((len(ReA_nms), len(ReC_nms)), np.nan, dtype=float)
    best_base = np.full_like(CR_best, np.nan)
    best_kind = np.empty(CR_best.shape, dtype=object)

    for i, A_scale in enumerate(A_scales):
        for j, ReC in enumerate(ReC_nms):
            best_cr = -1.0
            best_b = None
            best_k = None
            for A_base in A_bases:
                for A_kind in A_kinds:
                    ...
                    if cr > best_cr:
                        ...
                        best_k = str(A_kind)
            CR_best[i, j] = best_cr
            best_base[i, j] = best_b if best_b is not None else np.nan
            best_kind[i, j] = best_k if best_k is not None else ""

    # long-form CSV
    rows = []
    for i, ReA_nm in enumerate(ReA_nms):
        for j, ReC_nm in enumerate(ReC_nms):
            rows.append({
                "A_scale": float(A_scales[i]),
                "ReA_nm": float(ReA_nm),
                "ReC_nm": float(ReC_nm),
                "best_CR": float(CR_best[i, j]),
                "best_A_base_deg": float(best_base[i, j]),
                "best_A_kind": str(best_kind[i, j]),
            })
    df_long = pd.DataFrame(rows)
    df_long.to_csv(outdir / f"ReA_ReC_map_{tag}.csv", index=False)

    # pivot/grid CSV
    df_grid = df_long.pivot(index="ReA_nm", columns="ReC_nm", values="best_CR").sort_index()
    df_grid.to_csv(outdir / f"ReA_ReC_grid_{tag}.csv", index=True)

    # plot contour
    X, Y = np.meshgrid(ReC_nms, ReA_nms)  # X=ReC, Y=ReA
    Z = CR_best.copy()
    if use_log_cr:
        Zp = np.log10(np.maximum(Z, 1e-12))
        zlabel = "log10(CR)"
    else:
        Zp = Z
        zlabel = "CR"
    ...
    ax = fig.add_subplot(111)
    cs = ax.contourf(X, Y, Zp, levels=20)
    cbar = fig.colorbar(cs, ax=ax)
    cbar.set_label(zlabel)
    ax.set_xlabel("ReC (nm)")
    ax.set_ylabel("ReA (nm)")
    ax.set_title(f"{tag} CR map at (theta={theta_tgt:.1f}°, phi={phi_tgt:.1f}°)")
    ax.grid(True, ls=":", alpha=0.4)
    fig.tight_layout()
    fig.savefig(outdir / f"ReA_ReC_map_{tag}.png", bbox_inches="tight")
    plt.close(fig)

    return df_long, df_grid
...
def main():
    ...
    p.add_argument("--export_ReA_ReC_map", action="store_true",
                   help="export 2D CR map at target angle over (ReA, ReC) grids (CSV + contour PNG)")
    p.add_argument("--map_linear_cr", action="store_true",
                   help="plot CR (linear) in ReA-ReC contour map (default: log10(CR))")
    ...
    if args.load_best_from is not None:
        ...
        for key in keys_to_run:
            ...
            if bool(args.export_envelope) or bool(args.export_Astep_iso) or bool(args.export_ReA_ReC_map):
                envelope_over_A(cd['builder'], A_scales, ReC_nms,
                                cd['A_bases'], cd['A_kinds'],
                                float(args.theta), float(args.phi), c1, c2, outdir, cd['tag'])
                if bool(args.export_ReA_ReC_map):
                    export_ReA_ReC_map(cd['builder'], A_scales, ReC_nms,
                                      cd['A_bases'], cd['A_kinds'],
                                      float(args.theta), float(args.phi), c1, c2,
                                      outdir, cd['tag'], use_log_cr=(not bool(args.map_linear_cr)))
                if bool(args.export_ReA_ReC_map):
                    export_ReA_ReC_map(cd["builder"], A_scales, ReC_nms,
                                      cd["A_bases"], cd["A_kinds"],
                                      float(args.theta), float(args.phi), c1, c2,
